 ...
 */
const VALID_ROLES = ['quan_ly', 'phu_huynh', 'tai_xe'];
...
class AuthService {
    ...
    async register(userData) {
        ...
        const usernameExists = await AuthRepo_1.default.isUsernameExists(userData.ten_tai_khoan);
        if (usernameExists) {
            throw new Error('Tên tài khoản đã tồn tại');
        }
        // Kiểm tra số điện thoại đã tồn tại (nếu có)
        if (userData.so_dien_thoai) {
            const phoneExists = await AuthRepo_1.default.isPhoneExists(userData.so_dien_thoai);
            if (phoneExists) {
                throw new Error('Số điện thoại đã được sử dụng');
            }
        }
        // Băm mật khẩu
        const mat_khau_bam = await bcrypt_1.default.hash(userData.mat_khau, SALT_ROUNDS);
        // Tạo user mới
        const newUser = await AuthRepo_1.default.createUser({
    ...
    async login(credentials) {
        ...
        const user = await AuthRepo_1.default.findUserByUsername(credentials.ten_tai_khoan);
        if (!user) {
            throw new Error('Tên tài khoản hoặc mật khẩu không đúng');
        }
        // So sánh mật khẩu
        const isPasswordValid = await bcrypt_1.default.compare(credentials.mat_khau, user.mat_khau_bam);
        if (!isPasswordValid) {
            throw new Error('Tên tài khoản hoặc mật khẩu không đúng');
        }
        // Trả về thông tin user (không bao gồm password)
     ...
     */
    async getUserById(id_nguoi_dung) {
        const user = await AuthRepo_1.default.findUserById(id_nguoi_dung);
        if (!user) {
            return null;
        }
        return {
     ...
     * Thay đổi mật khẩu
     */
    async changePassword(id_nguoi_dung, mat_khau_cu, mat_khau_moi) {
        if (mat_khau_moi.length < MIN_PASSWORD_LENGTH) {
            throw new Error(`Mật khẩu mới phải có ít nhất ${MIN_PASSWORD_LENGTH} ký tự`);
        }
        const user = await AuthRepo_1.default.findUserByUsername(''); // Cần implement findById with password
        if (!user) {
            throw new Error('Người dùng không tồn tại');
        }
        const isPasswordValid = await bcrypt_1.default.compare(mat_khau_cu, user.mat_khau_bam);
        if (!isPasswordValid) {
